...
//------------------------Parameter----------------------
`define TV_IN_weights "../tv/cdatafile/c.ViT_act.autotvin_weights.dat"
`define TV_OUT_weights "../tv/rtldatafile/rtl.ViT_act.autotvout_weights.dat"
 parameter weights_ADDR_BITWIDTH = 32'd 64;
 parameter weights_AWUSER_BITWIDTH = 32'd 1;
 parameter weights_DATA_BITWIDTH = 32'd 256;
...
parameter weights_C_DATA_BITWIDTH = 32'd 256;
parameter weights_mem_depth = 32'd 235020;
...
reg [weights_DATA_BITWIDTH - 1 : 0] weights_mem_3 [0: weights_mem_depth - 1] = '{default : 'h0}; 
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > weights_mem_depth) begin
                $display ("E:/DeiTCPP/Deit_cpp/solution1/sim/verilog/AESL_axi_master_weights.v: Write request address %d exceed AXI master weights array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, weights_mem_depth); 
                $finish;
            end
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        ...
                                        3 : WDATA_tmp[j] = weights_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            ...
                            3 : weights_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        ...
                                        3 : WDATA_tmp[j] = weights_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        default: $display("The page_num of AXI write is not valid!");
                                    endcase
                                end
                            end
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            ...
                            3 : weights_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > weights_mem_depth) begin
                $display ("E:/DeiTCPP/Deit_cpp/solution1/sim/verilog/AESL_axi_master_weights.v: Read request address %d exceed AXI master weights array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, weights_mem_depth); 
                $finish;
            end
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            ...
                            3 : RDATA_tmp <= weights_mem_3[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            ...
                            3 : RDATA_tmp <= weights_mem_3[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            default: $display("The page_num of AXI read is not valid!");
                        endcase
                    RVALID_tmp <= 1;
                    RRESP_tmp <= 0;
...
initial begin : read_file_process 
  ...
  integer mem_page; 
  integer weights_bitwidth; 
  reg [256 - 1 : 0] token_tmp; 
  reg [weights_DATA_BITWIDTH - 1 : 0] mem_tmp; 
  integer i; 
  transaction_num = 0;
  weights_bitwidth = weights_C_DATA_BITWIDTH; 
  count_seperate_factor_by_bitwidth (weights_bitwidth , factor);
      ...
      for(i = 0; i < weights_mem_depth; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  case(mem_page)
                      0 : weights_mem_0[i/factor] = mem_tmp;
                      1 : weights_mem_1[i/factor] = mem_tmp;
                      2 : weights_mem_2[i/factor] = mem_tmp;
                      3 : weights_mem_3[i/factor] = mem_tmp;
          ...
          if (factor == 1) begin
              mem_tmp = token_tmp;
              case(mem_page)
                  0 : weights_mem_0[i] = mem_tmp;
                  1 : weights_mem_1[i] = mem_tmp;
                  2 : weights_mem_2[i] = mem_tmp;
                  3 : weights_mem_3[i] = mem_tmp;
                  default: $display("The page_num of read file is not valid!");
              endcase
              mem_tmp [weights_DATA_BITWIDTH - 1: 0] = 0;
          end
