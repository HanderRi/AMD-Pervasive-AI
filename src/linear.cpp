        ...
        )
        {
            ...
            FOR_EACH(row, LINEAR_OUT_SIZE)
            {
                ...
                FOR_BLOCK(col, LINEAR_IN_SIZE, WEIGHT_BLOCK_SIZE)
                {
                    ...
                    FOR_OFFSET(col)
                    {
                        row_for_dst[col] = cached[col_offset];//将权重缓存写入目标权重数�?
                    }
                }
...
)
{
    ...
    compute_linear_on_stream(out_stream, in_stream, weights, bias, out_dim, in_dim, NUM_PATCHES, use_gelu);
