...
void load_norms(wt_norm_t norm_weights[NUM_LAYER_NORMS][FEATURE_DIM], wt_bias_t norm_bias[NUM_LAYER_NORMS][FEATURE_DIM])
{
    ...
    wt_bias_t bias[FEATURE_DIM])
{
    ...
    fm_t variance = mean_sq - sq_mean + norm_eps;//方差 平方的均�?-均�?�的平方
    fm_t rstddev = fm_t(1)/hls::sqrt(variance);// 1/标准�?
