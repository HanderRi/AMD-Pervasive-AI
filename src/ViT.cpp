...
)
{
            ...
            FOR_EACH(y, PATCH_HEIGHT)
            {
                FOR_BLOCK(dim_out,FEATURE_DIM,FEATURE_BLOCK_SIZE)
                {
                    FOR_OFFSET_NOCHK(dim_out)
                    {
                        ...
                        {
                            #pragma HLS occurence cycle=dim_out_step
                            ...
                            FOR_EACH(dim_out_write_offset,dim_out_step)
                            {
                                ...
                                {
                                    patch_embed_weights[dim_out_write][channel][y][x] = weights_cache[dim_out_offset][x];
                                }
    ...
    attn_scale = 0.125;//注意力缩放因�? 计算注意力得分时，会将计算出的得分乘�? 0.125，以防止数�?�过大�?�导致的梯度爆炸或计算不稳定
    norm_eps = 1e-6;//防止在归�?化时出现除以零的情况 进行归一化计算时，如果标准差非常小，norm_eps 会被加到标准差上以避免除以零的情�?
 }

extern "C"{
    void ViT_act(
        unsigned int num_images,//图片数量
        ...
        wt_linear_t attn_weights[NUM_LAYERS][NUM_ATTN_LINEAR][FEATURE_DIM][FEATURE_DIM],//注意力层的权�?
        wt_attn_bias_t attn_bias[NUM_LAYERS][NUM_ATTN_LINEAR][FEATURE_DIM],//注意力层的偏�?
        
        wt_linear_t vit_weights_l1[max_ab((NUM_LAYERS+1)/2,1U)][VIT_HIDDEN_DIM][FEATURE_DIM],
        wt_bias_t vit_bias_l1[max_ab((NUM_LAYERS+1)/2,1U)][VIT_HIDDEN_DIM],
        wt_linear_t vit_weights_l2[max_ab((NUM_LAYERS+1)/2,1U)][FEATURE_DIM][VIT_HIDDEN_DIM],
        wt_bias_t vit_bias_l2[max_ab((NUM_LAYERS+1)/2,1U)][FEATURE_DIM],
    ...
    )
    {
        ...
        {
            load_one_time_weights(patch_embed_bias,patch_embed_weights);//权重加载
        }

        FOR_EACH(image,num_images)//每个图像
        {
            #pragma HLS loop_tripcount min=1 max=1 avg=1//tripcount手动指定循环迭代次数
            #pragma HLS pipeline off

            compute_patch_embed(images[image],x[image],pos_embed);//补丁嵌入

            FOR_EACH(layer,NUM_LAYERS)//网络每一�?
            {
                load_norms(norm_weights[layer],norm_bias[layer]);//归一化权重和偏置
                compute_norm1(x[image],tmp1);//该层第一次归�?化计�? 结果保存tmp1 此为Transformer模块的输�?
                ...
                //计算Q向量 保存到tmp2
                compute_linear(reinterpret_cast<fm_block_t*>(tmp2),reinterpret_cast<fm_block_t*>(tmp1),linear_weights_ping,linear_bias_ping,FEATURE_DIM,FEATURE_DIM,false);
                ...
                //计算K向量 保存到tmp3
                compute_linear(reinterpret_cast<fm_block_t*>(tmp3),reinterpret_cast<fm_block_t*>(tmp1),linear_weights_pong,linear_bias_pong,FEATURE_DIM,FEATURE_DIM,false);

                //计算QxK 进行softmax 结果保存attn
                ...
                compute_add(x[image],tmp3,x[image]);//残差连接
                ...
                compute_linear(tmp_hidden,reinterpret_cast<fm_block_t*>(tmp1),linear_weights_ping,linear_bias_ping,VIT_HIDDEN_DIM,FEATURE_DIM,true);//用到gelu模块
                ...
                compute_linear(reinterpret_cast<fm_block_t*>(tmp3),tmp_hidden,linear_weights_pong,linear_bias_pong,FEATURE_DIM,VIT_HIDDEN_DIM,false);                
